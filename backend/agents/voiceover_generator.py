...
class VoiceoverGenerator(BaseAgent):
    ...
    # MiniMax Speech-02 is the persona voice cast target. gTTS remains as a
    # zero-config fallback for local development without API keys.
    TTS_IMPORTS = {
        "gtts": "from manim_voiceover.services.gtts import GTTSService",
        "minimax": "from agents.voice_services import MiniMaxService",
    }
    ...
    TTS_SETUP = {
        ...
        "minimax": (
            "self.set_speech_service(MiniMaxService("
            "persona='feynman', model='speech-02-hd', transcription_model=None))"
        ),
    }
    ...
    def __init__(
        ...
        Args:
            ...
            tts_service: TTS service to use ("minimax" or "gtts"). When None,
                defaults to "minimax" if MINIMAX_API_KEY is set, otherwise "gtts".
        ...
        if tts_service is None:
            tts_service = "minimax" if os.environ.get("MINIMAX_API_KEY") else "gtts"
