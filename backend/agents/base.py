...
# Default model — Moonshot Kimi K2.6 routed through the Dedalus gateway.
# To override per-deployment without touching code, set
# DEFAULT_MODEL_OVERRIDE in the environment (e.g. "deepseek/deepseek-chat").
DEFAULT_MODEL = os.environ.get("DEFAULT_MODEL_OVERRIDE", "moonshotai/kimi-k2-instruct")
...
def _dedalus_model(model: str) -> str:
    """Normalise a bare model name to a Dedalus provider/model string.

    The pipeline targets sponsor providers by default. If a caller passes a
    bare model name (no slash), prefix it with the sponsor provider so
    routing works without further configuration. Callers that want a
    specific provider already pass `provider/model` strings.
    """
    if "/" in model:
        return model
...
class BaseAgent:
    ...
    def __init__(
        ...
        # Log active provider
        print(f"🔮 Dedalus SDK → {_dedalus_model(self.model)}")
